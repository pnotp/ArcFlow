...
[train_flux.sh](train_flux.sh) provides a training example using DDP on one node with 4 GPUs. This requires ~65GB of GPU memory for each GPU. To train with multi-nodes, run the following command:
```bash
torchrun --nnodes={num_of_nodes} --nproc_per_node={num_of_gpus_per_node} --node_rank=<NODE_RANK> --master_addr=<MASTER_ADDR> --master_port=<MASTER_PORT> train.py configs/flux/arcflux_2nfe_k16.py --launcher pytorch --diff_seed
```
