...
[train_qwen.sh](train_qwen.sh) provides a training example using DDP on one node with 4 GPUs. To train with multi-nodes, run the following command:
```bash
torchrun --nnodes={num_of_nodes} --nproc_per_node={num_of_gpus_per_node} --node_rank=<NODE_RANK> --master_addr=<MASTER_ADDR> --master_port=<MASTER_PORT> tools/train.py configs/qwen/arcqwen_2nfe_k16.py --launcher pytorch --diff_seed
```
